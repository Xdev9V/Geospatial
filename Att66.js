define(["https://api.tiles.mapbox.com/mapbox-gl-js/v1.2.0/mapbox-gl.js", "jquery", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v2.2.0/mapbox-gl-geocoder.min.js","https://api.tiles.mapbox.com/mapbox.js/plugins/turf/v3.0.11/turf.min.js","https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.0.9/mapbox-gl-draw.js", "https://ajax.aspnetcdn.com/ajax/jQuery/jquery-3.2.1.min.js", "https://code.jquery.com/ui/1.12.1/jquery-ui.min.js","text! https://cdn.jsdelivr.net/gh/Xdev9/Geospatial/Mapbox_12.css"], function(mapboxgl, jQuery, MapboxGeocoder, turf, MapboxDraw, slider1, slider2, Info) {
    ...
    function myFunction() {
	 ...
	 var visibility = map.getLayoutProperty('Att_Claims', 'visibility');

        if ('visibility' === 'visible') {
            map.setLayoutProperty('Att_Claims', 'visibility', 'none');
...
// Change the cursor to a pointer when the mouse is over the places layer.
    ...
    map.on('mouseleave', 'Earthquake - Historical', function () {
		...
		map.on('load', function () {
    map.addSource('Att_Claims', {
        type: 'vector',
        url: 'mapbox://fxhawk.ck4mukdab1hr32op8ytw5z1jc-2tjvx'
    });
	map.addLayer({
        'id': 'Att_Claims',
        'type': 'circle',
        'source': 'Att_Claims',
		'source-layer': 'Att_Claims',
		 "layout": {
                "visibility": property,
		...
		map.on('click', function(e) {
  var features = map.queryRenderedFeatures(e.point, {
    layers: ['Att_Claims'] // replace this with the name of the layer
  });
  ...
  var popup = new mapboxgl.Popup({ offset: [0, -15] })
    .setLngLat(feature.geometry.coordinates)
    .setHTML('<h3>' +'Claim Number: ' + feature.properties.Claim_Number + '</h3>'  + 'DOL: ' +  feature.properties.DOL + '<br>' + 'Status: ' + feature.properties.Status + '<br>' + 'Total Paid: ' + feature.properties.Total_Paid +   '</p>')
    .setLngLat(feature.geometry.coordinates)

    .addTo(map);
});
// Change the cursor to a pointer when the mouse is over the places layer.
    map.on('mouseenter', 'Att_Claims', function () {
        map.getCanvas().style.cursor = 'pointer';
    });

    // Change it back to a pointer when it leaves.
    map.on('mouseleave', 'Att_Claims', function () {
        map.getCanvas().style.cursor = '';
    });
...
if (e.type !== 'draw.delete') alert("Use the draw tools to draw a polygon!");
    ...
    BasicControl.prototype.setData = function(oControlHost, oDataStore) {
        ...
        for (var iRow = 0; iRow < iRowCount; iRow++) {
            ...
            feature['properties'] = { 'name': oDataStore.getCellValue(iRow, 0), 'score': oDataStore.getCellValue(iRow, 14) ,'firm': oDataStore.getCellValue(iRow, 15) }
...
var Firm = e.features[0].properties.firm;	
...
.setHTML('<p><strong>' + 'Attorney: ' + Claim_N + '</strong></p>' + '<br><strong>' + 'Firm: ' + Firm + '<br>' + 'Letter Score: ' + Score + '<br>' + 'Score: ' + Score + '</strong></p>' )
	.addTo(map);
});
