...
// Corona*****************************************************************************************
		...
		map.on('load', function () {
    map.addSource('Locations_7', {
        type: 'vector',
        url: 'mapbox://fxhawk.ck84pi7ya01672rof8nenwatx-1b297'
    });
	map.addLayer({
        'id': 'Locations_7',
        'type': 'symbol',
        'source': 'Locations_7',
		'source-layer': 'Locations_7',
		 "layout": {
                "visibility": 'visible',
                "text-field": [
                    "to-string",
		...
		map.on('click', function(e) {
  var features = map.queryRenderedFeatures(e.point, {
    layers: ['Locations_7'] // replace this with the name of the layer
  });
...
// Change the cursor to a pointer when the mouse is over the places layer.
    map.on('mouseenter', 'Locations_7', function () {
        map.getCanvas().style.cursor = 'pointer';
    });

    // Change it back to a pointer when it leaves.
    map.on('mouseleave', 'Locations_7', function () {
        map.getCanvas().style.cursor = '';
    });
 ...
 map.on('draw.create', function (e){
        ...
        var features = map.queryRenderedFeatures([southWestPointPixel, northEastPointPixel], { layers: ['Locations_7'] });
      ...
      window.aggr = features.reduce(function (memo,feature, Paid3) {
            ...
            if (! (undefined === turf.intersect(feature, userPolygon))) {
              ...
              var Paid3 = feature.properties.Num_of_Employees;
...
answer.innerHTML = '<p><strong>' + 'Number of employees: ' + window.aggr.toLocaleString() + '</strong></p>';
} else {
answer.innerHTML = '';
if (e.type !== 'draw.delete') alert("Use the draw tools to draw a polygon!");
    ...
    BasicControl.prototype.setData = function(oControlHost, oDataStore) {
        ...
        for (var iRow = 0; iRow < iRowCount; iRow++) {
            ...
            feature['properties'] = { 'name': oDataStore.getCellValue(iRow, 0), 'total_paid3': oDataStore.getCellValue(iRow, 17), 'street': oDataStore.getCellValue(iRow, 16), 'Sqft': oDataStore.getCellValue(iRow, 19),  'Num_of_Employees': oDataStore.getCellValue(iRow, 18)  }
			    ...
			    year_built = oDataStore.getCellValue(iRow, 15)
			    street = oDataStore.getCellValue(iRow, 16)
			    Num_of_Employees = oDataStore.getCellValue(iRow, 18)
			    Sqft = oDataStore.getCellValue(iRow, 19)
...
map.on('click', function(e) {
  ...
  var popup = new mapboxgl.Popup({ offset: [0, -15] })
    .setLngLat(feature.geometry.coordinates)
    .setHTML('<h3>' + feature.properties.name + '</h3>'  + 'Street: ' +  feature.properties.street + '<br>' + 'Property Value: ' + feature.properties.total_paid3 + '<br>' + 'Sqft:  ' + feature.properties.Sqft   + '<br>' + 'Number of Employees: ' + feature.properties.Num_of_Employees +  '</p>')
    .setLngLat(feature.geometry.coordinates)
