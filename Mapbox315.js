define(["https://api.tiles.mapbox.com/mapbox-gl-js/v1.2.0/mapbox-gl.js", "jquery", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v2.2.0/mapbox-gl-geocoder.min.js","https://api.tiles.mapbox.com/mapbox.js/plugins/turf/v3.0.11/turf.min.js","https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.0.9/mapbox-gl-draw.js", "https://ajax.aspnetcdn.com/ajax/jQuery/jquery-3.2.1.min.js", "https://code.jquery.com/ui/1.12.1/jquery-ui.min.js","https://cdn.jsdelivr.net/gh/Xdev9/Geospatial/Mapbox_11.css"], function(mapboxgl, jQuery, MapboxGeocoder, turf, MapboxDraw, slider, slider1, sCSS) {
    ...
    BasicControl.prototype.initialize = function(oControlHost, fnDoneInitializing, oDataStore) {
		...
		jQuery("tbody").prepend("<link rel='stylesheet' href='https://cdn.jsdelivr.net/gh/Xdev9/Geospatial/Mapbox_11.css' rel='stylesheet' />");
		 //jQuery("tbody").prepend("<link rel='stylesheet' href='https://cdn.jsdelivr.net/gh/Xdev9/Geospatial/Mapbox_9.css' rel='stylesheet' />");
	...
	var sHtml = '<div id=' + 'console' + '>' +  '</div>' + '<div class=' + 'session' + ' ' +  'id=' + 'sliderbar' + ' ' + '>' + ' ' + '<h5>' + '<font color=' + "black" + '>' + 'Historical - Year: ' + '<label id=' + 'active-Year' + '>' + '2013' + '</label>' + '</font>' +
	'<input id=' + 'slider' + ' ' + 'class=' +'row' + ' ' + 'type=' + 'range' + ' ' + 'min=' + '2013' + ' ' + 'max='+ '2019' + ' ' +  'step=' + '1' + ' ' + 'value=' + '2013'  + '/>' + '</h5>' + '</div>';
...
var myFeatures = map.querySourceFeatures ('markers2');
  var feature = features[0];
...
// Change the cursor to a pointer when the mouse is over the places layer.
 ...
 // var Year = parseInt(e.target.value);
  ...
  // map.setFilter('Claims', ['==', ['number', ['get','Year']], Year]);
