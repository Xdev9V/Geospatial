...
map.on('click', function(e) {   
  ...
  var features = map.queryRenderedFeatures(e.point, {
    layers: ['Wildfire_point'] // replace this with the name of the layer
  });
 ...
 var feature = features[0];
 var Day = new Date(feature.properties.CreateDate);
 var Day1 = Day.toUTCString();

  var popup = new mapboxgl.Popup({ offset: [0, -15] })
    .setLngLat(feature.geometry.coordinates)
    .setHTML('<h3>' + feature.properties.IncidentName + '</h3><p>' + 'GISAcres: ' + feature.properties.GISAcres + '<br>' + 'Date: ' + Day1 + '<br>' + 'source: National Interagency Fire Center' + '</p>')
    .setLngLat(feature.geometry.coordinates)
    .addTo(map);

});
// Change the cursor to a pointer when the mouse is over the places layer.
    map.on('mouseenter', 'Wildfire_point', function () {
        map.getCanvas().style.cursor = 'pointer';
    });	  	    

  // Change it back to a pointer when it leaves.
    map.on('mouseleave', 'Wildfire_point', function () {
        map.getCanvas().style.cursor = '';
    });
