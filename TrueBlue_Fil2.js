define(["https://api.mapbox.com/mapbox-gl-js/v2.9.2/mapbox-gl.js", "jquery", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v5.0.0/mapbox-gl-geocoder.min.js", "https://api.mapbox.com/mapbox.js/plugins/turf/v3.0.11/turf.min.js", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.3.0/mapbox-gl-draw.js", "https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js",/*"https://code.jquery.com/ui/1.12.1/jquery-ui.min.js",*/ "text! https://cdn.jsdelivr.net/gh/nelnawawy-ventiv/geotest/Demobase7.css"], function (mapboxgl, jQuery, MapboxGeocoder, turf, MapboxDraw, slider1, slider2, Info) {
    ...
    BasicControl.prototype.initialize = function (oControlHost, fnDoneInitializing, oDataStore) {
        ...
        document.getElementById("v162_ValueComboBox").addEventListener("change", myFunction11);
        ...
        document.getElementById("v174_ValueComboBox").addEventListener("change", myFunction12);
        ...
        document.getElementById("v185_ValueComboBox").addEventListener("change", myFunctionUSTEMP);

        function myFunctionUSTEMP() {

            var visibility = map.getLayoutProperty('US Temp', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('US Temp', 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('US Temp', 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }
        ...
        map.on('load', function () {
                ...
                for (var i = 0; i < frameCount; i++) {
                    ...
                    map.addLayer({
                        ...
                        'source': {
                            'type': 'raster',
                            'tiles': [
                                'https://idpgis.ncep.noaa.gov/arcgis/rest/services/NWS_Forecasts_Guidance_Warnings/NDFD_temp/MapServer/export?dpi=96&bbox=-293.7692491671929,-328.4203216326983,80.39101937471492,439.0178934058135&layers=show:127&transparent=true&format=png32&bboxSR=EPSG:3857&imageSR=EPSG:3857&size=256,256&f=image'
                            ],
                            'tileSize': 256
                        },
            ...
            map.on('load', function () {
                map.addLayer({
                    ...
                    "source": {
                        "type": "raster",
                        "tiles": ['https://idpgis.ncep.noaa.gov/arcgis/rest/services/NWS_Forecasts_Guidance_Warnings/NDFD_temp/MapServer/export?dpi=96&bbox=-293.7692491671929,-328.4203216326983,80.39101937471492,439.0178934058135&layers=show:127&transparent=true&format=png32&bboxSR=EPSG:3857&imageSR=EPSG:3857&size=256,256&f=image'],
                        "tileSize": 256
    ...
    BasicControl.prototype.setData = function (oControlHost, oDataStore) {
        ...
        for (var iRow = 0; iRow < iRowCount; iRow++) {
            ...
            feature['properties'] = { 'name': oDataStore.getCellValue(iRow, 0), 'total_paid3': oDataStore.getCellValue(iRow, 22), 'cause': oDataStore.getCellValue(iRow, 19), 'customer': customer = oDataStore.getCellValue(iRow, 20), 'record_link': oDataStore.getCellValue(iRow, 21) }
            ...
            ustempout = oDataStore.getCellValue(iRow, 18)
        ...
        map.on('click', 'points', function (e) {
            ...
            new mapboxgl.Popup()
                ...
                .setHTML('<h3>' + 'Claim: ' + Claim_N + '</h3>' + 'Cause: ' + cause + '<br>' + 'Customer: ' + customer + '<br>' + 'Developed Claim Cost: ' + paidf + '<br>' + '<a href=' + hyperlink/*e.features[0].properties.RECORD_LINK.value*/ + ' target="_blank">' + 'View Details' + '</a>' + '</p>')
                .addTo(map);
        });
