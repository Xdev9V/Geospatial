define(["https://api.tiles.mapbox.com/mapbox-gl-js/v1.2.0/mapbox-gl.js", "jquery", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v2.2.0/mapbox-gl-geocoder.min.js","https://api.tiles.mapbox.com/mapbox.js/plugins/turf/v3.0.11/turf.min.js","https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.0.9/mapbox-gl-draw.js", "https://ajax.aspnetcdn.com/ajax/jQuery/jquery-3.2.1.min.js", "https://code.jquery.com/ui/1.12.1/jquery-ui.min.js","text! https://cdn.jsdelivr.net/gh/Xdev9/Geospatial/Mapbox_12.css"], function(mapboxgl, jQuery, MapboxGeocoder, turf, MapboxDraw, slider1, slider2, Info) {
    ...
    function myFunction() {
	 ...
	 var visibility = map.getLayoutProperty('Locations_3', 'visibility');

        if (sessionStorage.getItem('visibility') === 'visible') {
            map.setLayoutProperty('Locations_3', 'visibility', 'none');
			sessionStorage.setItem('visibility','none');
			//document.getElementById("v30_ValueComboBox").style.backgroundColor = '#9BA3A5';

        } else {

            map.setLayoutProperty('Locations_3', 'visibility', 'visible');
...
// Corona*************************************************************************************
    ...
    map.addSource('cv_data_all', {
        type: 'vector',
        url: 'mapbox://fxhawk.ck7m6ci9m00it2tn13mmhtcsj-0b6rj'
    });
	map.addLayer({
        'id': 'corona_all',
         "type": "circle",
            "source": "cv_data_all",
            "source-layer": "cv_data_all",
            "layout": {
                "visibility": corona
            },
            "paint": {
...
// Corona*****************************************************************************************
		...
		map.on('load', function () {
    map.addSource('Locations_3', {
        type: 'vector',
        url: 'mapbox://fxhawk.ck7mbsr3h02xp2kqy1toms4ci-57lqh'
    });
	map.addLayer({
        'id': 'Locations_3',
        'type': 'symbol',
        'source': 'Locations_3',
		'source-layer': 'Locations_3',
		 "layout": {
                "visibility": property,
                "text-field": [
                    "to-string",
		...
		map.on('click', function(e) {
  var features = map.queryRenderedFeatures(e.point, {
    layers: ['Locations_3'] // replace this with the name of the layer
  });
...
// Change the cursor to a pointer when the mouse is over the places layer.
    map.on('mouseenter', 'Locations_3', function () {
        map.getCanvas().style.cursor = 'pointer';
    });

    // Change it back to a pointer when it leaves.
    map.on('mouseleave', 'Locations_3', function () {
        map.getCanvas().style.cursor = '';
    });
